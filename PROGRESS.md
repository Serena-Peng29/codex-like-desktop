...
- 2026-08-28：附件改为按上游协议传路径、审批请求全部接通（`apps/desktop/src/turn-input.ts`、`main.ts`、`preload.cts`、`types.d.ts`、`renderer/main.tsx`）：删除 `expandLocalFileInputs` 客户端内联文件内容的逻辑（附件内容曾被整段写进消息文本并渲染出来），替换为 `buildTurnInputItems`——图片继续走 `localImage` 路径项（core 自行读文件编码），文件附件折叠为消息文本内的路径行并用上游 `text_elements`（`byteRange` UTF-8 字节偏移 + `placeholder` 文件名）标记；字段命名经固定上游 schema 核对（v2 `UserInput::Text` 用 `text_elements`，`TextElement` 用 `byteRange`）。历史恢复统一走 `userMessageParts`：从 textElements 还原附件芯片、剥离路径行与 `Attached files:` 头，并剥离旧版 `[Attached local file: ...]`/`<file>`/`<binary>` 内联块使老会话不再整段显示文件内容；`localImage`/`mention`/data-url 项处理保持不变。审批侧：`item/fileChange/requestApproval`、`item/permissions/requestApproval`、`item/tool/requestUserInput`、`mcpServer/elicitation/request` 全部路由到审批卡片（此前只处理 commandExecution，文件改动/权限审批无人应答导致 turn 永久卡住）；`app-server:approval-response` 按方法校验并构造上游响应结构：command/fileChange `{decision: accept|acceptForSession|decline|cancel}`、permissions `{permissions: 回显请求项或空 profile 表示拒绝, scope: turn|session}`、userInput `{answers:{}}`、elicitation `{action, content:null}`；卡片按类型显示命令/写入目录（`grantRoot`）/权限摘要/reason，并提供允许、本会话允许、拒绝、取消按钮。验证：`npm test`（3 文件 10 用例，`buildTurnInputItems` 新增 6 用例覆盖 UTF-8 字节偏移、多附件顺序、结构化 mention 透传）、`npm run typecheck`、`npm run startup-check`、`npm run protocol-schema-check` 通过。未回归：真实 sidecar 下 fileChange/permissions 审批交互与 textElements 历史回放待正常桌面环境复测。
- 2026-08-28：修复审批 UI 不可见导致无法点击同意的问题（`apps/desktop/src/renderer/main.tsx`、`styles.css`）：审批/差异面板此前错误地被 `tool === "files"` 条件限制，收到 App Server 审批请求后不会渲染；现按工具类型正确显示审批面板，批准按钮使用高对比绿色主按钮，窄窗口下主批准操作独占一行且操作区自动换行。验证：`npm test`（3 个测试文件、10 个用例）、`npm run build:all`、`npm run startup-check` 通过；Playwright CLI 因当前 Windows 沙箱 bash/npm 缓存权限不可用，未完成浏览器截图复测。
- 2026-08-28：修复会话状态与重复助手气泡（`apps/desktop/src/main.ts`、`types.d.ts`、`renderer/main.tsx`）：`app:state` 暴露当前运行中的 `activeTurnThreadId/activeTurnId`，启动和切换会话时恢复运行中占位及发送状态；流式事件、工具事件和审批状态按线程绑定，避免切换会话后串写到当前对话；历史恢复及当前渲染合并同一轮连续的助手消息，避免审批前进度和审批后最终回复显示成两个助手气泡。验证：`npm run typecheck`、`npm test`（3 个测试文件、10 个用例）、`git diff --check` 通过。
- 2026-08-29：桌面端网关直连注入 P1-0（`apps/desktop/src/main.ts`，设计基线为本地 `docs/gateway-auth.md`，按约定不入库）：`SidecarManager.start` 支持 env 门控的模型网关透传——设置 `WAY2AGI_GATEWAY_URL`+`WAY2AGI_GATEWAY_TOKEN` 时，spawn 追加 `-c model_provider="way2agi"`、`model_providers.way2agi.{name,base_url,env_key}`、`disable_response_storage=true`（可选 `WAY2AGI_MODEL`），`CODEX_HOME` 隔离到 `<userData>/codex-home`，用户令牌只经子进程环境变量 `WAY2AGI_TOKEN` 传递，密钥不写 config.toml/auth.json（自定义 provider `requires_openai_auth=false` 时 env_key 是唯一认证来源，上游机制见 vendor `model-provider/src/auth.rs` 裁决顺序）。端到端验证：用 npm sidecar 二进制以相同 `-c` 参数直连 api1.aisz.mom `/v1/responses`，模型流式回复成功，`thread/tokenUsage/updated` 回传 input 16725/output 20/cached 3840/reasoning 12；`npm run typecheck`、`npm test`（10 用例）、`npm run startup-check` 通过。临时探测脚本 `scripts/tmp-gateway-probe.mjs`、`scripts/tmp-tool-probe.mjs` 按临时文件规则不入库。未回归：`npm run dev` 下含文件修改与命令审批的完整交互会话；下一步 P1-1 网关重写为验 JWT + 转发 + usage 记账。
- 2026-08-29：修复新会话首条消息「一直思考中」（`apps/desktop/src/renderer/main.tsx`，用户 `npm run dev` 网关直连实测发现）：渲染层在流式期间用 `activeThreadIdRef` 关联 delta 与审批，但新会话首条消息时该 ref 为 null——更新它的 `state()` 探测与主进程 `thread/start` 存在竞态且早于线程创建完成，`stream()` 又要等 turn 结束才返回；导致 `item/agentMessage/delta` 全部被丢弃（正文不显示）、`item/fileChange/requestApproval` 到达时卡片只入库不弹出，turn 永远等待审批。诊断依据：同一 `-c` 参数的探测脚本完整跑通「创建 ptt.txt」回合（8.5s 首条 agentMessage、8.7s fileChange 审批、18.8s commandExecution 审批、30.4s 完成），排除网络与 sidecar 因素（直连中转站 0.8s 可达）。修复：`onMessageDelta` 在 ref 为 null 且存在本端发起的流式 assistant 时收养首个 delta 的 threadId；`onApproval` 在无法归属到其他线程（ref 为 null）时也显示审批卡片——审批不可见即 turn 死锁，宁可多显示。验证：`npm run typecheck`、`npm test`（10 用例）、`npm run startup-check` 通过；交互复测待用户环境（记入未解决问题）。

## 未解决问题

- 真实 sidecar 对 goal/plan、文件文本展开和跨平台路径行为待回归。
- 2026-08-29 修复后待复测：`npm run dev` 网关直连下新会话首条消息的完整审批交互（fileChange → commandExecution → 完成）。
