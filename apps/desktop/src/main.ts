...
async function runAppServerTurn(input: string, options?: { effort?: string }) {
  if (!projectPath) throw new Error("project_required");
  if (activeTurn) throw new Error("turn_already_running");
  if (!activeThreadId) {
    const result = await sidecar.request("thread/start", { cwd: projectPath, approvalPolicy: "on-request", sandbox: "workspace-write", ephemeral: true });
  ...
  return await new Promise<{ output: string; usage: Record<string, number> }>((resolvePromise, reject) => {
    ...
    void sidecar.request("turn/start", { threadId: activeThreadId, cwd: projectPath, approvalPolicy: "on-request", sandboxPolicy: { type: "workspaceWrite", writableRoots: [projectPath] }, effort: options?.effort ?? "medium", input: [{ type: "text", text: input }] }).catch((error: unknown) => { if (activeTurn) { activeTurn = null; reject(error instanceof Error ? error : new Error(String(error))); } });
...
app.whenReady().then(async () => {
  ...
  sidecar.setRequestHandler((request) => {
    if (request.method === "item/commandExecution/requestApproval") mainWindow?.webContents.send("app-server:approval", { requestId: request.id, method: request.method, params: request.params ?? {} });
  });
  sidecar.setNotificationHandler((notification) => {
    const params = notification.params ?? {};
    if (!activeTurn) return;
    if (notification.method === "item/agentMessage/delta" && params.threadId === activeTurn.threadId && typeof params.delta === "string") { activeTurn.output += params.delta; mainWindow?.webContents.send("app-server:message-delta", params.delta); }
  ...
  ipcMain.handle("app:state", () => ({ projectPath, sidecar: sidecar.status, gateway: gatewayUrl, gatewayMode: gatewayIsRemote ? "remote" : "local" }));
  ipcMain.handle("project:choose", async () => { const result = await dialog.showOpenDialog({ properties: ["openDirectory", "createDirectory"] }); projectPath = result.canceled ? projectPath : result.filePaths[0] ?? projectPath; if (projectPath) await sidecar.request("workspace/set", { path: projectPath }).catch(() => undefined); return projectPath; });
