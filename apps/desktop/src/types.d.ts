...
declare global {
  interface Window {
    desktop: {
      ...
      state(): Promise<{ projectPath: string | null; activeThreadId: string | null; unassignedThreadIds?: string[]; threadProjectPaths?: Record<string, string | null>; history: Array<{ id: string; preview?: string; name?: string | null; cwd?: string; updatedAt?: number; createdAt?: number; ephemeral?: boolean; status?: unknown }>; sidecar: string; gateway: string; gatewayMode: "remote" | "local" }>;
      ...
      stream(input: Array<{ type: "text"; text: string } | { type: "localImage"; path: string }>, options?: { effort?: string; planMode?: boolean }): Promise<{ output: string; usage: Record<string, number> }>;
      interrupt(): Promise<boolean>;
      chooseFiles(): Promise<Array<{ path: string; name: string; preview?: string }>>;
