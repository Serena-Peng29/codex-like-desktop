...
function tokens(text) { return Math.max(1, Math.ceil(text.length / 4)); }
export function createGatewayServer() {
    return createServer(async (req, res) => {
        if (req.method === "GET" && req.url === "/health")
            return json(res, 200, { ok: true, service: "model-gateway" });
        if (req.method === "GET" && req.url?.startsWith("/v1/accounts/")) {
            const id = req.url.split("/").pop() ?? "test-user";
            try {
                return json(res, 200, ledger.get(id));
            }
            catch {
        ...
        if (!charge.ok)
            return json(res, 402, { error: "insufficient_credits", account: ledger.get(userId) });
        if (body.model && body.model !== "gpt-4o-mini")
            return json(res, 400, { error: "model_not_available" });
